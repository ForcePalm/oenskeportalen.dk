        ...
        <?php 
            if($wish->is_reserved){
                if($wish->reserved_by == $this->request->getAttribute('identity')->getIdentifier()){
                    ?>
                        <a class="default-hover-effect" href="<?= $this->Url->build(['controller' => 'Wishes', 'action' => 'cancel', $wish->uuid]) ?>">Annuller reservation</a>
                    <?php
                }else{
                    ?>
                        <button disabled class="disabled default-hover-effect">Reserveret</button>
                    <?php
                }
            }else{
                ?>
                    <a class="default-hover-effect" href="<?= $this->Url->build(['controller' => 'Wishes', 'action' => 'reserve', $wish->uuid]) ?>">Reserver</a>
                <?php
            }            
        ?>
    </div>   
